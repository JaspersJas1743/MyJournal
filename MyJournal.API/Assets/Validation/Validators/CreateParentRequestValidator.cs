...
public sealed class CreateParentRequestValidator : AbstractValidator<AdministratorController.CreateParentRequest>
{
	public CreateParentRequestValidator()
	{
		...
		RuleFor(expression: request => request.ChildrenId)
			.GreaterThan(valueToCompare: 0).WithMessage(errorMessage: "Идентификатор подопечного должен быть больше 0.");
	}
}