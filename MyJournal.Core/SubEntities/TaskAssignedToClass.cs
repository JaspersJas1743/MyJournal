...
public sealed class TaskAssignedToClass : BaseTask
{
	...
	internal static async Task<TaskAssignedToClass> Create(
		...
		IFileService fileService,
		GetCreatedTasksResponse response
	) => new TaskAssignedToClass(client: client, fileService: fileService, response: response);
	...
	)
	{
		...
		return new TaskAssignedToClass(client: client, fileService: fileService, response: response);
	}
	#endregion
